...
The offline test suite covers everything in `psn_monitor.py` that can run without network access. PlayStation Network calls are replaced with test doubles, so the suite needs no PSN account, no network and no credentials.
...
The test layout mirrors the surfaces a user touches rather than the module layout. See [tests/README.md](https://github.com/misiektoja/psn_monitor/blob/main/tests/README.md) for the file-by-file map.
...
* Never use a real npsso code, SMTP password, webhook URL or ntfy access token.

A change to the monitoring loop, authentication or PSN data handling is not verified by this suite alone. Exercise it against a real account and say so in the pull request, without usernames or credentials.
