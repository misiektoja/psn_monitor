...
* **Configuration**: the configuration and dotenv files in use, which secrets are loaded and where each came from, the time zone, the check intervals, whether TLS verification is on and the files the tool would write
...
* **Notifications**: whether email and webhook alerts can fire, and whether the settings each would use are valid
