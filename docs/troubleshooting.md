...
* `VERBOSE_MODE`, `--verbose`: operational events, such as an alert channel switched off because its settings are still placeholders, whether an email was actually delivered and what a liveness banner means. It prints nothing per check, so an uneventful run stays quiet. It also expands the startup summary, which is where the configuration file, dotenv file, time zone and the source of each secret are named
