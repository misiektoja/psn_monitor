...
def is_too_many_open_files(ex):
    for cur in iter_exc_chain(ex):
        ...
        if getattr(cur, "response", None) is not None:
            continue
        msg = str(cur).lower()
        if "too many open files" in msg or re.search(r"oserror\(24[,)]", msg) or re.search(r"\berrno 24\b", msg):
            return True
    return False
...
COMMAND_PLACEHOLDERS = frozenset(("<psn_user_id>", "<npsso_code>", "<new-file>"))
...
def encode_non_ascii_header_value(value):
    text = str(value)
...
def psn_monitor_user(psn_user_id, csv_file_name):
    ...
    while True:
        ...
        try:
            ...
            # The retry policy says how many failing checks confirm this kind of failure, and the reporter counts every
            # one of them, so the reminder and the recovery line cover the whole outage rather than the reported part
            outage.confirm_checks = policy["report_after"]
...
# unusable can be reported and then ignored instead of stopping the commands that exist to correct it
BUILT_IN_SHAPE_SETTINGS = {name: globals()[name] for name in ('PSN_LOGFILE', 'PSN_STATUS_FILE', 'CSV_FILE', 'DOTENV_FILE', 'COLOR_THEME', 'TRUNCATE_CHARS') if name in globals()}
...
def command_reports_configuration(args=None):
    ...
    return any(getattr(args, name, False) for name in ("doctor", "setup", "set_npsso", "set_smtp_password", "set_webhook_url"))
...
def run_setup_wizard(initial_target=None, config_file=None, env_file=None, input_func=None, getpass_func=None, interactive=None):
    ...
    try:
        if doctor_offered and _wizard_ask_yes_no("Run doctor now? It writes no files and offers real delivery tests only with separate approval.", default=True, input_func=input_func):
            ...
            doctor_exit = run_doctor(psn_user_id=state.target, config_path=str(state.config_path), env_path=str(state.env_path) if state.env_path.is_file() else None, timezone_advice=timezone_advice)
    except (EOFError, KeyboardInterrupt):
