...
def runtime_command(command, prefix=None):
    module = importlib.import_module("psn_monitor")
    assert module.__file__ is not None
    manual = [sys.executable, str(Path(module.__file__).resolve())]
    packaged = [sys.executable, "-m", "psn_monitor"]
    ...
    command = re.sub(r"(?<![\w/])python(?:3(?:\.\d+)?)? psn_monitor\.py", lambda match: manual_prefix, command)
    return re.sub(r"(?<![\w/])psn_monitor(?= |$)", lambda match: packaged_prefix, command)
