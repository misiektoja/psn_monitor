...
monitor = importlib.import_module("psn_monitor")
...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/psn_monitor.py" if method == "manual" else "/private/long/bin/psn_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/psn_monitor.py")
    ...
    expected = executable + " psn_monitor.py --setup" if method == "manual" else "psn_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "psn_monitor.py --setup" in result.stdout
