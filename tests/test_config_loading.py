...
HOSTILE_CONTENT = (
    "import os\n"
    "os.environ['PSN_CONFIG_EXEC_PROBE'] = 'yes'\n",
...
def test_executable_config_content_is_refused_without_running(tmp_path, monkeypatch, content):
    monkeypatch.delenv("PSN_CONFIG_EXEC_PROBE", raising=False)
    ...
    assert os.environ.get("PSN_CONFIG_EXEC_PROBE") is None
...
def test_config_file_none_disables_discovery(tmp_path):
    result = subprocess.run([sys.executable, str(PROJECT_ROOT / "psn_monitor.py"), "--config-file", "none", "--env-file", "none", "--no-color"], cwd=tmp_path, capture_output=True, text=True, check=False)
