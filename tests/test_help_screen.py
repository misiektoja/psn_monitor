...
def help_screen():
    result = subprocess.run([sys.executable, str(PROJECT_ROOT / "psn_monitor.py"), "--help"], cwd=PROJECT_ROOT, capture_output=True, text=True, check=False)
...
def test_the_wizard_is_the_first_example(help_screen):
    ...
    assert first == ["# Guided setup, recommended for the first run", runtime_command("python3 psn_monitor.py --setup")]
...
def test_the_description_links_the_repository(help_screen):
    ...
    assert "[ https://github.com/misiektoja/psn_monitor/ ]" in header
