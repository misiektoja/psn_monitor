...
# Ignores malformed color values so help remains available to repair configuration
def test_invalid_color_value_does_not_crash_help_styling():
    assert monitor._build_ansi_sequence(7) == ""
...
@pytest.mark.parametrize("value,expected", [("https://ntfy.sh/private-topic", "https://ntfy.sh/private-topic"), ("private-topic", "https://ntfy.sh/private-topic")])
# Recognizes ntfy destinations when the provider still has its shipped default
def test_private_ntfy_entry_works_with_default_provider(tmp_path, monkeypatch, value, expected):
    from dotenv import dotenv_values
    monkeypatch.setattr(monitor, "WEBHOOK_PROVIDER", "discord")
    path = tmp_path / ".env"
    monitor.run_set_webhook_url(env_file=str(path), interactive=True, getpass_func=lambda _: value)
    assert dotenv_values(path)["WEBHOOK_URL"] == expected


# Retains an explicit provider choice instead of silently changing its meaning
def test_private_destination_rejects_an_explicit_provider_mismatch(tmp_path, monkeypatch):
    monkeypatch.setattr(monitor, "WEBHOOK_PROVIDER", "discord")
    path = tmp_path / ".env"
    with pytest.raises(monitor.RecoveryError):
        monitor.run_set_webhook_url(env_file=str(path), interactive=True, getpass_func=lambda _: "https://ntfy.sh/private-topic", provider="discord")
...
def test_status_reader_keeps_legacy_trailing_metadata(tmp_path):
    path = tmp_path / "status.json"
    record = [1600000000, "offline", {"note": "kept"}]
...
def test_a_short_retained_secret_never_replaces_ordinary_words(monkeypatch):
    monkeypatch.setattr(monitor, "PSN_NPSSO", "cat", raising=False)
    monkeypatch.setattr(monitor, "WEBHOOK_URL", "https://discord.com/api/webhooks/123456789012345678/private")
