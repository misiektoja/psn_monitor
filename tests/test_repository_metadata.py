...
def test_citation_metadata_describes_this_project():
    ...
    assert citation["title"] == "psn_monitor"
    assert citation["message"]
    assert citation["license"] == "GPL-3.0-or-later"
    assert citation["repository-code"] == "https://github.com/misiektoja/psn_monitor"
...
def test_support_document_routes_every_request_type():
    ...
    for destination in ("https://github.com/misiektoja/psn_monitor/discussions", "https://github.com/misiektoja/psn_monitor/security/advisories/new", "https://github.com/misiektoja/psn_monitor/issues/new"):
        assert destination in support
    assert "psn_monitor --version" in support
    for concept in ("NPSSO key", "SMTP passwords", "webhook URLs",):
...
def test_local_hooks_run_the_pinned_linter():
    ...
    assert covered.match("psn_monitor.py")
    assert covered.match("tests/test_repository_metadata.py")
    assert "psn_monitor.py tests" in lint_command
