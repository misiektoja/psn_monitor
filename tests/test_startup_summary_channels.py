...
# Returns the summary rows a run with no files would print, keyed by label
def summary_values(**overrides):
    return {row.label: row.value for row in monitor.build_startup_summary("target", None, None, None, **overrides)}


# Returns those same rows rendered as the lines the summary prints, keyed by label
def summary_lines(**overrides):
    return {row.label: monitor.format_startup_summary_row(row) for row in monitor.build_startup_summary("target", None, None, None, **overrides)}


# Returns the labels of the rows a run with no files would print, in order
def summary_labels(**overrides):
    return [row.label for row in monitor.build_startup_summary("target", None, None, None, **overrides)]
...
def test_disabled_config_discovery_is_reported(monkeypatch):
    monkeypatch.setattr(monitor, "CONFIG_DISCOVERY_DISABLED", True)

    assert summary_values()["Config"] == "Discovery disabled"
...
def test_the_new_detail_rows_stay_out_of_the_concise_view():
    ...
    concise = {row.label for row in monitor.build_startup_summary("target", None, None, None) if row.concise}
...
# Verifies the tolerated disappearance is reported, since it decides how late an offline alert looks
def test_the_offline_grace_period_row_reports_the_setting(monkeypatch):
    monkeypatch.setattr(monitor, "OFFLINE_INTERRUPT", 420)

    assert summary_values()["Offline grace period"] == "7 minutes"
...
def test_a_channel_without_a_destination_is_reported_as_unavailable(monkeypatch):
    ...
    assert monitor.startup_webhook_notification_state().startswith("On (")
